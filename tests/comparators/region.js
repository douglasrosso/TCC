 ...
 * regiões com conteúdo dinâmico (datas, contadores, banners).
 *
 * Vantagem: reduz o impacto de deslocamentos e flutuações
 * de renderização, isolando diferenças em áreas específicas.
 */
...
/**
 * Calcula a fração de pixels diferentes em uma célula.
 * Um pixel é "diferente" quando qualquer canal difere mais que `channelTol`.
 */
...
/** Aplica uma cor semitransparente sobre uma célula na imagem de diff. */
...
/** Desenha borda retangular de uma célula. */
 ...
 * @param {string}   [options.diffPath]          Caminho para salvar imagem de diferenças.
 * @param {number}   [options.gridCols=4]        Colunas da grade.
 * @param {number}   [options.gridRows=6]        Linhas da grade.
 * @param {number}   [options.maxDiffPercent=5]   Percentual máximo por célula antes de falhar.
...
export async function compare(baselinePath, currentPath, options = {}) {
  ...
  if (diffPath) {
    ...
    baseline.data.copy(diff.data);             // cópia da baseline como fundo
