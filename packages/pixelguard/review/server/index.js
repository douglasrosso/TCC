...
const RESULTS_DIR =
  process.env.PIXELGUARD_RESULTS_DIR || path.resolve(process.cwd(), "results");
const BASELINES =
  process.env.PIXELGUARD_BASELINES_DIR ||
  path.resolve(process.cwd(), "baselines");
...
const PORT = portIdx >= 0 ? parseInt(process.argv[portIdx + 1], 10) : 8080;
...
function serveImage(res, filePath) {
  ...
  const ext = path.extname(filePath).toLowerCase();
  const mime =
    { ".png": "image/png", ".jpg": "image/jpeg", ".jpeg": "image/jpeg" }[ext] ||
...
async function handleRequest(req, res) {
  ...
  if (pathname.startsWith("/img/baseline/")) {
    serveImage(
      ...
      path.join(
        BASELINES,
        decodeURIComponent(pathname.replace("/img/baseline/", "")),
      ),
    );
    return;
  }
  if (pathname.startsWith("/img/current/")) {
    serveImage(
      ...
      path.join(
        CURRENT_DIR,
        decodeURIComponent(pathname.replace("/img/current/", "")),
      ),
    );
    return;
  }
  if (pathname.startsWith("/img/diff/")) {
    serveImage(
      ...
      path.join(
        RESULTS_DIR,
        "diffs",
        decodeURIComponent(pathname.replace("/img/diff/", "")),
      ),
    );
    return;
  }
  if (pathname.startsWith("/img/scenarios/baseline/")) {
    serveImage(
      ...
      path.join(
        SCENARIOS_DIR,
        "baseline",
        decodeURIComponent(pathname.replace("/img/scenarios/baseline/", "")),
      ),
    );
    return;
  }
  if (pathname.startsWith("/img/scenarios/current/")) {
    serveImage(
      ...
      path.join(
        SCENARIOS_DIR,
        "current",
        decodeURIComponent(pathname.replace("/img/scenarios/current/", "")),
      ),
    );
    return;
  }
  if (pathname.startsWith("/img/scenarios/diff/")) {
    serveImage(
      ...
      path.join(
        SCENARIOS_DIR,
        "diffs",
        decodeURIComponent(pathname.replace("/img/scenarios/diff/", "")),
      ),
    );
    return;
  }
  ...
  if (pathname === "/api/github/status" && req.method === "POST") {
    ...
    if (!fs.existsSync(metaPath)) {
      jsonResponse(
        res,
        { error: "meta.json não encontrado — este review não veio do CI" },
        400,
      );
      return;
    }
    ...
    if (!meta.repository || !meta.commitSha || meta.commitSha === "local") {
      jsonResponse(
        res,
        { error: "Review local — sem integração com GitHub" },
        400,
      );
      return;
    }
    ...
    if (!token) {
      jsonResponse(
        ...
        {
          error:
            "Nenhum token do GitHub configurado. Defina VRT_TOKEN, GH_TOKEN ou GITHUB_PAT no ambiente ou em .env",
        },
        401,
      );
      return;
    ...
    try {
      const apiRes = await fetch(
        `https://api.github.com/repos/${owner}/${repo}/statuses/${meta.commitSha}`,
        {
          method: "POST",
          headers: {
      ...
      if (!apiRes.ok) {
        ...
        jsonResponse(
          res,
          { error: "Erro na API do GitHub", details: err },
          apiRes.status,
        );
        return;
      }
